...
import { MdOutlineFormatListBulleted, MdWeb } from "react-icons/md";
import { useState } from "react";
import { Link } from "react-router-dom";
import Table, { ColumnsType } from "antd/es/table";
import moment from "moment";
import sslData from "../../_fakeData/sslData.json";
...
const columns: ColumnsType<any> = [
  ...
  },
  {
    ...
    render: (_, record) => {
      return (
        <Link to="#">
          <div className="line-clamp-1">{record?.sslProvider}</div>
        </Link>
      );
    },
  },
  {
    ...
    key: "vendor_logo",
    render: (_, record) => {
      return <div>{record?.validFrom}</div>;
    },
  },
  {
    title: "Valid Until",
    ...
    key: "vendor_logo",
    render: (_, record) => {
      return <div>{record?.validUntil}</div>;
    },
  },
  {
    title: "Days Left",
    ...
    <div>
      <div className="mb-4">
        <div className="flex justify-start items-center gap-3">
          ...
          <div className="text-[28px] font-medium text-primary">Blacklist</div>
        </div>
        <p>You have total 2,595 Blacklist.</p>
      </div>
      ...
      >
        <div>
          <h4 className="font-medium text-base">Blacklist Information</h4>
          <div className="mt-5">
            <div className="mb-4">
              <label htmlFor="name" className="input_label">
                Blacklist Name <span className="text-danger">*</span>
              </label>
              <input
                type="text"
                name="name"
